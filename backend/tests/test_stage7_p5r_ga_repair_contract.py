...
from tests.fixtures.stage7_p5r_ga_repair_cases import (
    RAW_RESUME,
    REFERENCE_AT,
...
@pytest.mark.parametrize("repair_choice", ("zero", "positive"))
def test_r04_frozen_raw_allows_both_llm_owned_legal_repairs(
    repair_choice: str,
) -> None:
    from scripts import run_stage7_p5r_f_r04_diagnostic as r04_runner

    inputs = r04_runner._r04_inputs()
    attempt = json.loads(r04_runner.ATTEMPT_PATHS[0].read_text(encoding="utf-8"))
    ...
    for assessment in repaired["criterion_assessments"]:
        if assessment["criterion_id"] not in broken_ids:
            continue
        if repair_choice == "zero":
            assessment["score"] = 0
            assessment["evidence"] = []
        else:
            assessment["evidence"] = [
                {
                    "quote": "AI 重新判断：材料提供了支持该低正分的有限依据。",
                    "section": "项目经历",
                }
            ]
    adapter = RepairCapableFakeAdapter(
        [adapter_result(raw)],
        [adapter_result(repaired)],
    )
    ...
    _assert_one_repair(adapter)
    repaired_by_id = {
        item.assessment.criterion_id: item.assessment
        for item in result.report.criterion_assessments
    }
    if repair_choice == "zero":
        assert all(
            repaired_by_id[criterion_id].score == 0
            and repaired_by_id[criterion_id].evidence == []
            for criterion_id in broken_ids
        )
    else:
        assert all(
            repaired_by_id[criterion_id].score in {1, 2}
            and len(repaired_by_id[criterion_id].evidence) == 1
            for criterion_id in broken_ids
        )
