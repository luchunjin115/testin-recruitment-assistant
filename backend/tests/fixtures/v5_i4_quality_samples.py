...
I4_LABEL_CONTRACT_VERSION = "stage7_v5_quality_contract_v3"
I4_STABILITY_RUNS_PER_SAMPLE = 3
...
def _plan(case_id: str, spec: dict[str, str]) -> dict[str, Any]:
    return {
        ...
        "label_contract_version": I4_LABEL_CONTRACT_VERSION,
        "labels": {
...
I4_PLAN_JDS = [
    _plan(f"I4-P{index:02d}", spec) for index, spec in enumerate(_ROLE_SPECS)
]
...
def _report_case(
    ...
    plan_case = I4_PLAN_JDS[plan_index]
    case_id = f"I4-R{report_index:02d}"
    if direction == "high_match":
        ...
        raise ValueError(f"unsupported I4 direction: {direction}")
    return {
        "case_id": case_id,
...
I4_REPORT_PAIRS: list[dict[str, Any]] = [
    _report_case(
        report_index,
        report_index % len(I4_PLAN_JDS),
        direction=direction,
    )
    for report_index, direction in enumerate(_REPORT_DIRECTIONS)
]

I4_STABILITY_SAMPLE_INDICES = [0, 4, 8, 10, 14]
...
def compute_i4_fixture_hashes() -> dict[str, str]:
    ...
    plan_labels = [case["labels"] for case in I4_PLAN_JDS]
    report_samples = [
        {
            key: value
    ...
    fixture = {
        "plan_jds": I4_PLAN_JDS,
        "report_pairs": I4_REPORT_PAIRS,
        "stability_indices": I4_STABILITY_SAMPLE_INDICES,
    ...
    return {
        ...
        "stability_selection": _sha256(I4_STABILITY_SAMPLE_INDICES),
    }
