...
JOB_REQUIREMENT_COVERAGE_REVIEW_PROMPT_VERSION = (
    "job_requirement_coverage_review_v1"
)
...
逐一审阅输入中的全部 source units。title、department、job_background 只提供上下文，不能产生事实。事实只能来自 job_responsibilities、candidate_requirements、preferred_qualifications。
...
能分别根据简历评价的语义应分别提取；拆开会破坏原意或拆散固定组合概念时保留。相同事实在多处出现时放入同一个 candidate 的 sources，不得仅因主题相近强行合并。模糊要求保留原文并在对应 review 标记 ambiguous_requirement，不补充年限、数量、等级、技能或强制语气。福利、介绍、宣传和招聘流程不生成事实，在评价字段中出现时标记 non_evaluation_content。
...
你只能审计 source units、最终候选 facts 和程序重算的 source review，不得改写 facts，不得生成 criterion、priority、计划状态或招聘决定。逐段检查是否存在：missing_fact、unsupported_fact、wrong_disposition、invalid_atomicity、missing_source_merge、category_mismatch。
