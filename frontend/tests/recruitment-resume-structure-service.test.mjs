...
try {
  ...
  const { structureRecruitmentResume } = await server.ssrLoadModule(
    '/src/features/recruitment/services/resumes.ts',
  );
  const requests = [];
  ...
  const first = await structureRecruitmentResume(32);
  const forced = await structureRecruitmentResume(32, true);
  ...
  await assert.rejects(
    structureRecruitmentResume(32),
    error => error === originalError,
  );

  console.log('RECRUITMENT_RESUME_STRUCTURE_SERVICE_TEST_OK');
} finally {
  await server.close();
}
