...
try {
  ...
  } = await server.ssrLoadModule('/src/stage3/resumeSupplementaryInfo.ts');
  ...
  const { createStage3Candidate } = await server.ssrLoadModule(
    '/src/stage3/services/candidates.ts',
  );
  ...
  console.log('STAGE3_RESUME_SUPPLEMENTARY_INFO_TEST_OK');
} finally {
  await server.close();
}
