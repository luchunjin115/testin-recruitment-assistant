...
export type Stage3JobCreateInput = Stage3JobInput & { status: 'draft' | 'open' };

export type Stage3JobApiError = {
  status: number | null;
  code: string | null;
  message: string;
  fields: string[];
...
const mapJob = (job: JobResponse, candidateCount = 0): Stage3Job => ({
  id: job.id,
  title: job.title,
...
export const createStage3Job = async (data: Stage3JobCreateInput): Promise<Stage3Job> => {
  const response = await v2Http.post<JobResponse>('/jobs', data);
  return mapJob(response.data);
};

export const updateStage3Job = async (jobId: number, data: Stage3JobInput): Promise<Stage3Job> => {
...
export const openStage3Job = (jobId: number) => runStatusAction(jobId, 'open');
export const closeStage3Job = (jobId: number) => runStatusAction(jobId, 'close');
export const reopenStage3Job = (jobId: number) => runStatusAction(jobId, 'reopen');

export const deleteStage3Job = async (jobId: number): Promise<void> => {
  await v2Http.delete(`/jobs/${jobId}`);
};

export const getStage3JobApiError = (error: unknown): Stage3JobApiError => {
