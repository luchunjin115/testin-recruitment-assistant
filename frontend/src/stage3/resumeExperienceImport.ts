import type {
  Stage3EducationInput,
  Stage3ProjectExperienceInput,
  Stage3WorkExperienceInput,
...
): ResumeExperienceCandidate<Stage3EducationInput>[] => drafts.map(draft => {
  const key = buildCandidateKey('education', draft);
  const formValue: Stage3EducationInput = {
    aiCandidateKey: key,
...
): ResumeExperienceCandidate<Stage3WorkExperienceInput>[] => drafts.map(draft => {
  const key = buildCandidateKey('work', draft);
  const formValue: Stage3WorkExperienceInput = {
    aiCandidateKey: key,
...
): ResumeExperienceCandidate<Stage3ProjectExperienceInput>[] => drafts.map(draft => {
  const key = buildCandidateKey('project', draft);
  const formValue: Stage3ProjectExperienceInput = {
    aiCandidateKey: key,
