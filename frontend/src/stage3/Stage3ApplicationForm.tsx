...
import { getStage3ApplicationJobs, Stage3ApplicationJob } from './services/application';
import './styles/index.css';

type LoadState =
  | { status: 'loading' }
  | { status: 'error'; message: string }
  | { status: 'ready'; jobs: Stage3ApplicationJob[] };
...
const Stage3ApplicationForm: React.FC = () => {
  ...
  const loadJobs = useCallback(async () => {
    ...
    try {
      setLoadState({ status: 'ready', jobs: await getStage3ApplicationJobs() });
    } catch (error) {
  ...
  const renderShell = (content: React.ReactNode) => (
    <div className="s3-apply-page">
      <header className="s3-apply-header">
        <Link className="s3-apply-brand" to="/stage3/apply" aria-label="HR Agent 候选人投递">
          <span className="s3-apply-brand-mark">HR</span>
          <span>
            <strong>HR Agent</strong>
            <small>候选人投递</small>
          </span>
        </Link>
        <Tag className="s3-apply-preview-tag" icon={<LockOutlined />}>表单结构预览 · 不会提交</Tag>
      </header>
      {content}
    </div>
  );
  ...
  if (loadState.status === 'loading') {
    return renderShell(
      <main className="s3-apply-main" aria-busy="true">
        <div className="s3-apply-loading-card">
  ...
  if (loadState.status === 'error') {
    return renderShell(
      <main className="s3-apply-main">
        <div className="s3-apply-state-card">
          <Alert
            showIcon
            type="error"
            message="开放岗位加载失败"
  ...
  return renderShell(
    <main className="s3-apply-main">
      <section className="s3-apply-intro" aria-labelledby="application-title">
        <Link className="s3-apply-back" to="/stage3/dashboard">
          <ArrowLeftOutlined /> 返回招聘工作台
        </Link>
        <div className="s3-apply-eyebrow"><span /> CANDIDATE APPLICATION</div>
        <h1 id="application-title">加入我们，开启下一段职业旅程</h1>
        <p>选择一个正在招聘的岗位，填写个人信息并准备简历。本页目前用于确认新版投递体验，内容不会发送到服务器。</p>
        <div className="s3-apply-job-count">
          <strong>{loadState.jobs.length}</strong>
          <span>个开放岗位来自新版岗位数据</span>
        </div>
      </section>

      <div className="s3-apply-layout">
        <aside className="s3-apply-guide" aria-label="投递说明">
          <div className="s3-apply-guide-title">
            <SafetyCertificateOutlined />
            <div>
              <strong>投递前须知</strong>
          ...
          <div className="s3-apply-boundary-note">
            <LockOutlined />
            <span><strong>数据保护说明</strong>提交入口尚未开放，本页不会创建候选人、简历或 AI 结果。</span>
          </div>
        </aside>

        <section className="s3-apply-form-card" aria-label="候选人投递表单">
          {!hasOpenJobs && (
            <div className="s3-apply-empty-jobs">
          ...
          {hasOpenJobs && (
            <Form<ApplicationFormValues>
              ...
              autoComplete="off"
            >
              <div className="s3-apply-section-heading">
                <span><UserOutlined /></span>
                <div><strong>基本信息</strong><p>用于后续招聘团队与候选人取得联系</p></div>
              </div>
              <div className="s3-apply-grid">
                ...
                <Form.Item className="s3-apply-grid-full" label="电子邮箱" name="email" rules={[{ type: 'email', message: '请输入有效邮箱地址' }]}>
                  <Input placeholder="name@example.com" />
                </Form.Item>
              </div>

              <div className="s3-apply-divider" />
              <div className="s3-apply-section-heading">
                <span><BookOutlined /></span>
                <div><strong>教育背景</strong><p>填写最高学历对应的院校与专业信息</p></div>
              </div>
              <div className="s3-apply-grid s3-apply-grid-three">
              ...
              <Upload.Dragger {...uploadProps} className="s3-apply-uploader">
              ...
              <div className="s3-apply-submit-row">
...
export default Stage3ApplicationForm;
