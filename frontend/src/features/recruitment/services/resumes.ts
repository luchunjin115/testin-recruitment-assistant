...
export type RecruitmentResumeDetail = {
  id: number;
  candidateId: number | null;
  jobId: number | null;
  filename: string;
...
const mapResumeDetail = (resume: ResumeResponse): RecruitmentResumeDetail => ({
  id: resume.id,
...
export const getRecruitmentResumes = async (): Promise<ResumeListSnapshot> => {
...
export const uploadRecruitmentResume = async (file: File): Promise<RecruitmentResumeDetail> => {
...
export const extractRecruitmentResumeText = async (
  resumeId: number,
...
export const abandonRecruitmentResume = async (resumeId: number): Promise<void> => {
  await v2Http.delete(`/resumes/${resumeId}`);
};

export const getRecruitmentCandidateResumeFiles = async (
  candidateId: number,
...
export const getRecruitmentResumeFileUrl = (
  resumeId: number,
  download = false,
