...
const RecruitmentResumeList: React.FC = () => {
  ...
  const loadResumes = useCallback(async () => {
    ...
    try {
      const data = await getRecruitmentResumes();
  ...
  if (loadState.status === 'loading') {
    return (
      <main aria-busy="true" aria-label="简历列表加载中" className="recruitment-main">
    ...
    return (
      <main className="recruitment-main">
        <section className="recruitment-page-heading">
          <div><span className="recruitment-section-kicker">新版简历库 · 数据来源 /api/v2</span><h2>简历管理</h2><p>集中查看简历文件和解析状态。</p></div>
        </section>
        <Alert
  ...
  return (
    <main className="recruitment-main">
      <section className="recruitment-page-heading">
        <div>
          <span className="recruitment-section-kicker">新版简历库 · 数据来源 /api/v2</span>
          <h2>简历管理</h2>
      ...
      <section aria-label="简历状态统计" className="recruitment-stat-grid">
        {stats.map(stat => (
      ...
      <section className="recruitment-panel recruitment-resume-panel">
        <div className="recruitment-resume-toolbar">
          <div>
            <h3>简历列表</h3>
            <p>按上传时间倒序排列，共 {data.total} 份真实记录</p>
          </div>
          <div className="recruitment-filter-controls">
            <Input
              allowClear
              aria-label="搜索简历"
        ...
        {data.total === 0 ? (
          <Empty
            className="recruitment-panel-empty recruitment-resume-empty"
            description={
          ...
          <div aria-label="新版简历列表" className="recruitment-table" role="table">
            <div className="recruitment-table-head recruitment-resume-table-columns" role="row">
              <span>简历文件</span><span>候选人</span><span>关联岗位</span><span>文件信息</span><span>解析状态</span><span>上传时间</span>
            </div>
            {filteredItems.map(item => (
              <div className="recruitment-table-row recruitment-resume-table-columns" key={item.id} role="row">
                <div className="recruitment-resume-file-cell">
                  <span className="recruitment-file-icon"><FileTextOutlined /></span>
                  <div><strong title={item.filename}>{item.filename}</strong><span>简历 #{item.id}</span></div>
                </div>
                <div className="recruitment-resume-secondary">
                  <strong>{item.candidateName}</strong>
                  <span>{item.candidateId === null ? '尚未创建候选人' : `候选人 #${item.candidateId}`}</span>
                </div>
                <span className="recruitment-role-cell">{item.jobTitle}</span>
                <div className="recruitment-resume-secondary"><strong>{item.fileType?.toUpperCase() || '类型未记录'}</strong><span>{formatFileSize(item.fileSize)}</span></div>
                <div>
                  <Tag bordered={false} className={`recruitment-status-tag is-${statusMeta[item.parseStatus].tone}`}>
                    {item.parseStatus === 'parsed' && <CheckCircleOutlined />} {statusMeta[item.parseStatus].label}
                  </Tag>
                  {item.parseError && <span className="recruitment-parse-error" title={item.parseError}>查看失败原因</span>}
                </div>
                <span className="recruitment-time-cell">{formatDateTime(item.uploadedAt)}</span>
              </div>
            ))}
          </div>
        )}
...
export default RecruitmentResumeList;
