...
import {
  ...
  RecruitmentResumeDetail,
  structureRecruitmentResume,
...
type ResumeWorkflow =
  ...
  | { status: 'extracting'; resume: RecruitmentResumeDetail }
  | { status: 'ready'; resume: RecruitmentResumeDetail }
  | { status: 'failed'; message: string; resume?: RecruitmentResumeDetail };
...
const ResumeExperienceCandidateTray = <TFormValue,>({
  ...
  return (
    <div className="recruitment-candidate-ai-tray">
      <div className="recruitment-candidate-ai-tray-heading">
        <div>
          <span>AI 识别候选</span>
      ...
      <p className="recruitment-candidate-ai-tray-guidance">
        {hasExistingRecords
          ? '表单已有记录，AI 不会自动拼接。请勾选确认需要追加的条目。'
          : '这些内容尚未进入表单。你可以导入全部，也可以先勾选部分条目。'}
      </p>
      <div className="recruitment-candidate-ai-candidates">
        {candidates.map(candidate => {
          const imported = importedKeys.includes(candidate.key);
          const checked = selectedKeys.includes(candidate.key);
          return (
            <label
              className={`recruitment-candidate-ai-candidate ${imported ? 'is-imported' : ''}`}
              key={candidate.key}
            >
              <Checkbox
              ...
              />
              <span className="recruitment-candidate-ai-candidate-copy">
                <span className="recruitment-candidate-ai-candidate-title">
                  <strong>{candidate.title}</strong>
                  <span>{candidate.subtitle}</span>
                </span>
                {candidate.details.map(detail => <small key={detail}>{detail}</small>)}
                {candidate.tags.length > 0 && (
                  <span className="recruitment-candidate-ai-candidate-tags">
      ...
      {availableCandidates.length > 0 ? (
        ...
        <span className="recruitment-candidate-ai-tray-complete"><CheckCircleOutlined /> 这组 AI 候选已全部导入</span>
      )}
    </div>
  );
};
...
const ResumeSkillCandidateTray: React.FC<ResumeSkillCandidateTrayProps> = ({
  ...
  return (
    <div className="recruitment-candidate-ai-tray recruitment-candidate-ai-skill-tray">
      <div className="recruitment-candidate-ai-tray-heading">
        <div>
          <span>AI 识别候选</span>
      ...
      <div className="recruitment-candidate-ai-skill-candidates">
        {candidates.map(candidate => {
          const imported = importedKeys.includes(candidate.key);
          ...
          return (
            <label
              className={`recruitment-candidate-ai-skill ${imported || alreadyExists ? 'is-imported' : ''}`}
              key={candidate.key}
            >
              <Checkbox
                checked={checked}
      ...
      {availableCandidates.length > 0 ? (
        ...
        <span className="recruitment-candidate-ai-tray-complete"><CheckCircleOutlined /> 技能候选均已处理</span>
      )}
    </div>
  );
};
...
const ResumeSupplementaryPanel: React.FC<{ info: ResumeSupplementaryInfo }> = ({ info }) => (
  <section aria-label="简历识别补充信息" className="recruitment-candidate-supplementary">
    <div className="recruitment-candidate-supplementary-heading">
      <strong>识别补充信息</strong>
      <Tag bordered={false}>只读，不写入候选人字段</Tag>
    </div>
    {!info.hasContent ? (
      <p className="recruitment-candidate-supplementary-empty">本次识别没有额外证书、自我评价或需要核对的警告。</p>
    ) : (
      <div className="recruitment-candidate-supplementary-list">
        {info.warnings.length > 0 && (
          <div className="recruitment-candidate-supplementary-item is-warning">
            <strong><WarningOutlined /> 需要 HR 核对</strong>
            <ul>{info.warnings.map(item => <li key={item}>{item}</li>)}</ul>
          </div>
        )}
        {info.certifications.length > 0 && (
          <div className="recruitment-candidate-supplementary-item">
            <strong>证书</strong>
            <div className="recruitment-candidate-supplementary-tags">
...
const ResumeStructureTiming: React.FC<{
  ...
  <section aria-label={fromCache ? '缓存读取耗时' : '本次识别耗时'} className="recruitment-candidate-structure-timing">
    <div className="recruitment-candidate-structure-timing-heading">
    ...
    {fromCache ? (
      ...
      <div className="recruitment-candidate-structure-timing-grid">
...
const RecruitmentCandidateCreate: React.FC = () => {
  const navigate = useNavigate();
  const [form] = Form.useForm<RecruitmentCandidateCreateInput>();
  ...
  const loadJobs = useCallback(async () => {
    ...
    try {
      setJobState({ status: 'ready', jobs: await getRecruitmentCandidateJobs() });
    } catch (error) {
      setJobState({
        status: 'error',
  ...
  const abandonAttachedResume = async () => {
    ...
    setAbandoning(true);
    try {
      await abandonRecruitmentResume(attachedResume.id);
  ...
  const structureResume = async (resume: RecruitmentResumeDetail, force = false) => {
    setStructureWorkflow({ status: 'processing', force });
    try {
      const result = await structureRecruitmentResume(resume.id, force);
  ...
  const handleFormValuesChange = (changedValues: Partial<RecruitmentCandidateCreateInput>) => {
  ...
  const renderBasicFieldConflict = (fieldName: ResumeBasicFieldName) => {
    ...
    return (
      <span className="recruitment-candidate-field-conflict">
  ...
  const confirmRestructure = (resume: RecruitmentResumeDetail) => {
  ...
  const confirmAbandon = (navigateAfter: boolean) => {
    if (resumeBusy || submitting || abandoning) return;
    if (!attachedResume) {
      if (navigateAfter) navigate('/app/candidates');
      return;
    }
  ...
  const extractResume = async (resume: RecruitmentResumeDetail) => {
    setResumeWorkflow({ status: 'extracting', resume });
    try {
      const parsedResume = await extractRecruitmentResumeText(resume.id);
  ...
  const uploadResume = async (file: File) => {
    ...
    try {
      const uploadedResume = await uploadRecruitmentResume(file);
  ...
  const handleSubmit = async (values: RecruitmentCandidateCreateInput) => {
  ...
  if (jobState.status === 'loading') {
    return (
      <main aria-busy="true" className="recruitment-main recruitment-candidate-create-page">
        <section className="recruitment-page-heading"><Skeleton active paragraph={{ rows: 2 }} /></section>
        <section className="recruitment-state-panel"><Skeleton active paragraph={{ rows: 12 }} /></section>
      </main>
    );
  }
    ...
    return (
      <main className="recruitment-main recruitment-candidate-create-page">
        <section className="recruitment-page-heading recruitment-candidate-create-heading">
          ...
          <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/app/candidates')}>返回列表</Button>
        </section>
        <Alert
  ...
  return (
    <>
      ...
      <main className="recruitment-main recruitment-candidate-create-page">
        <section className="recruitment-page-heading recruitment-candidate-create-heading">
          <div className="recruitment-candidate-create-heading-copy">
            <Button
              aria-label="返回候选人列表"
              className="recruitment-candidate-create-back"
        ...
        >
          <div className="recruitment-candidate-create-layout">
            <div className="recruitment-candidate-create-main">
              <section className="recruitment-candidate-form-card">
                <div className="recruitment-candidate-form-card-header">
              ...
              <Form.List name="educationRecords">
                {(fields, { add, remove }) => (
                  <section className="recruitment-candidate-form-card">
                    ...
                    {fields.length === 0 ? (
                      <p className="recruitment-candidate-record-empty">暂无教育经历，按需添加</p>
                    ) : (
                      <div className="recruitment-candidate-record-list">
                        {fields.map((field, index) => (
                          <div className="recruitment-candidate-record" key={field.key}>
                            <div className="recruitment-candidate-record-title">
                            ...
                            <div className="recruitment-candidate-form-grid is-three">
                              ...
                              <div className="recruitment-candidate-school-flags">
              ...
              <Form.List name="workExperiences">
                {(fields, { add, remove }) => (
                  <section className="recruitment-candidate-form-card">
                    ...
                    {fields.length === 0 ? (
                      <p className="recruitment-candidate-record-empty">暂无工作经历，按需添加</p>
                    ) : (
                      <div className="recruitment-candidate-record-list">
                        {fields.map((field, index) => (
                          <div className="recruitment-candidate-record" key={field.key}>
              ...
              <Form.List name="projectExperiences">
                {(fields, { add, remove }) => (
                  <section className="recruitment-candidate-form-card">
                    ...
                    {fields.length === 0 ? (
                      <p className="recruitment-candidate-record-empty">暂无项目经历，按需添加</p>
                    ) : (
                      <div className="recruitment-candidate-record-list">
                        {fields.map((field, index) => (
                          <div className="recruitment-candidate-record" key={field.key}>
            ...
            <aside className="recruitment-candidate-create-aside">
              <section className="recruitment-candidate-resume-card">
                <div className="recruitment-candidate-resume-heading">
                  <span><FileTextOutlined /></span>
                  <div><h3>简历智能识别</h3><p>上传后自动读取简历内容并识别候选人信息</p></div>
                </div>

                {resumeWorkflow.status === 'idle' && (
                  <Upload.Dragger {...uploadProps} className="recruitment-candidate-uploader">
                ...
                {resumeWorkflow.status === 'uploading' && (
                  <div aria-live="polite" className="recruitment-candidate-resume-progress">
                    <Spin />
                ...
                {resumeWorkflow.status === 'ready' && (
                  <div className="recruitment-candidate-resume-result">
                    <div className="recruitment-candidate-resume-track" aria-label="简历处理进度">
                    ...
                    <div className="recruitment-candidate-resume-meta">
                    ...
                    {structureWorkflow.status === 'processing' && (
                      <div aria-live="polite" className="recruitment-candidate-structure-progress">
                        <Spin size="small" />
                        <div>
                    ...
                    {structureWorkflow.status === 'succeeded' && structureSummary && (
                      <div className="recruitment-candidate-structure-state is-success" aria-live="polite">
                        <Alert
                        ...
                        />
                        <div className="recruitment-candidate-structure-summary">
                    ...
                    {structureWorkflow.status === 'failed' && (
                      <div className={`recruitment-candidate-structure-state ${structureWorkflow.previousResult ? 'is-previous' : 'is-failed'}`} aria-live="polite">
                        <Alert
                    ...
                    <Collapse
                      className="recruitment-candidate-raw-text"
                      items={[{
                        children: (
                          <div className="recruitment-candidate-raw-text-content">
                ...
                {attachedResume && (
                  <p className="recruitment-candidate-resume-retention">
...
export default RecruitmentCandidateCreate;
