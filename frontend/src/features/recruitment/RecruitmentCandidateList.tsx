...
const RecruitmentCandidateList: React.FC = () => {
  ...
  const loadCandidates = useCallback(async () => {
    ...
    try {
      const data = await getRecruitmentCandidates();
  ...
  if (loadState.status === 'loading') {
    return (
      <main aria-busy="true" aria-label="候选人列表加载中" className="recruitment-main">
    ...
    return (
      <main className="recruitment-main">
        <section className="recruitment-page-heading">
          <div><span className="recruitment-section-kicker">新版候选人库 · 数据来源 /api/v2</span><h2>候选人</h2><p>集中查看候选人资料和岗位关联。</p></div>
        </section>
        <Alert
  ...
  return (
      ...
      <section className="recruitment-page-heading">
        <div>
          <span className="recruitment-section-kicker">新版候选人库 · 数据来源 /api/v2</span>
          <h2>候选人</h2>
      ...
      <section aria-label="候选人统计" className="recruitment-stat-grid">
        {stats.map(stat => (
      ...
      <section className="recruitment-panel recruitment-candidate-list-panel">
        ...
        {data.total === 0 ? (
          ...
          <div aria-label="新版候选人列表" className="recruitment-table" role="table">
            ...
            {filteredItems.map(item => {
              ...
              return (
                <div className="recruitment-table-row recruitment-candidate-table-columns" key={item.id} role="row">
                  <div className="recruitment-candidate-cell">
                    <Avatar className={`recruitment-candidate-avatar is-${avatarTones[item.id % avatarTones.length]}`}>
                      {item.name.slice(0, 1)}
                    </Avatar>
                    <div>
                      <Link className="recruitment-candidate-name-link" to={`/app/candidates/${item.id}`}>{item.name}</Link>
                      <span>{item.email || item.phone || `候选人 #${item.id}`}</span>
                    </div>
                  </div>
                  <span className="recruitment-role-cell">{item.appliedJobTitle}</span>
                  <div className="recruitment-candidate-experience">
                  ...
                  <span className="recruitment-role-cell">{item.source || '来源未填写'}</span>
