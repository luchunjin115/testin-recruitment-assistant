...
阶段 8 已进入需求确认门禁，但独立专项设计尚未建立，生产实现尚未授权。启动讨论时先阅读产品路线和实施计划；确认用户场景、主/异常流程、范围、数据/状态/权限/API、队列与 Worker、幂等重试、失败隔离、可观察性和验收方案后，再创建阶段 8 设计并更新本索引。
