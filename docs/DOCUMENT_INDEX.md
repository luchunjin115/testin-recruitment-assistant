...
第一份记录阶段 7 已实现并继续保留的 Application、Resume、报告、异步运行、时间事实和 HR 决策底座；第二份记录旧输入合同下的小步骤 9 和历史诊断；第三、四份分别保存 3.0 与 4.0 的实现事实、失败和真实调用证据；第五份是当前唯一新增业务合同，采用“完整 JD + HR 可编辑轻量评价清单 + 无权重 0—100 报告”；第六份只维护 2026-08-30 之后从当前状态到阶段完成评审的剩余顺序和执行看板。修改阶段 7 前必须完整阅读第五份和第六份，并按改动范围补充阅读第一份保留能力、4.0 历史实现、阶段 6 五段式补充和相关代码测试。
