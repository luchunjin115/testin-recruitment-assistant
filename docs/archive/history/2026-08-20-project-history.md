# 项目历史归档（截至 2026-08-20）

> 本文件由原 `PROJECT_STATE.md` 完整迁入，用于追溯阶段 0—7 的开发过程、验证记录和旧演示系统历史。当前进度与下一步请读取 `../../../PROJECT_STATE.md`；正文中的旧路径保留当时语境，不再作为现行导航。本文件不具有当前阶段业务权威性。

> 最新更新：2026-08-20（阶段 7 评分方案已完成业务重评审；当前只更新权威文档，下一小步先盘点并删除旧 Rubric 体系）
...
- 阶段 7 当前重设计与执行计划：`docs/specs/2026-08-20-stage7-jd-driven-ai-screening-redesign.md`
- 阶段 7 旧方案历史记录：`docs/specs/2026-08-17-stage7-application-ai-screening-design.md`（Rubric 与评分部分已废弃）
...
### 2026-08-20 阶段 7 评分方案重设计（文档门禁进行中）

- 用户逐项确认以 JD 为唯一岗位事实来源：岗位发布后先由 DeepSeek 拆解自由文本，再由程序补齐结构化 JobRequirements，合并为稳定、只读的 JD 基础评价事项；HR 不再配置或发布 Rubric。
- 旧 Rubric、五维权重、内部占比、AI 优化占比、Python 确定性评分、`unknown`、证据覆盖率、加权总分和多份完整报告历史全部废弃；先盘点并删除旧体系与旧业务数据，再开发新链路。
- 基础事项按 `required/preferred/general` 分类并逐项 `0—10` 分；0 分表示当前简历未体现，不代表候选人不会。DeepSeek 结合全部事项直接给出 `0—100` AI 岗位匹配建议分，Python 不重新加权。
- 每份简历允许 0—5 个有证据、7—10 分、只产生正向影响的“候选人额外亮点”；AI 不得在 JD 之外创造扣分标准。
- AI 返回严格 JSON，后端校验事项完整性、证据定位、敏感信息和明显矛盾，React 渲染完整初筛报告。AI 不生成通过、淘汰或录用建议，HR 决策继续独立。
- 每个 Application 只保留最近一次成功报告；重新评估成功后替换，失败时旧报告保持不变，仅保留必要运行日志。首次满足条件时后台自动初筛，支持最多 20 人小批量重新评估。
- 新权威设计为 `docs/specs/2026-08-20-stage7-jd-driven-ai-screening-redesign.md`。本文档复核通过前不得修改业务代码；当前工作区已有旧 Rubric 未提交改动，后续必须先只读扫描并形成精确删除清单，严禁 reset、clean 或盲删。
...
- 当前分支为 `2lcj`，本轮开始前最新提交为 `d12f01e`；新对话仍必须先检查 `git status`、`git log` 和 `git diff`，严禁 reset、clean、checkout 覆盖或丢弃现有修改。
...
- 阶段 7 旧 Rubric 方向已于 2026-08-20 废弃。Application、Resume 隔离、HR 录入和 HR 决策公共能力保留；下一小步先复核新阶段设计，再只读盘点旧 Rubric 前端到 PostgreSQL 引用并形成删除清单，不再实现“放弃草稿”或扩展旧评分编辑器。
...
### 阶段 7：Application 与 AI 初筛底座（进行中：评分方案已重设计）

> 2026-08-20 起，当前业务与验收标准以 `docs/specs/2026-08-20-stage7-jd-driven-ai-screening-redesign.md` 为准。下方关于旧 Rubric 小步骤的详细记录仅用于追溯已经发生的实现和验证，不得继续作为后续开发方向。

#### 已确认的业务边界

- `Candidate` 表示人员，`Application` 表示其对一个具体岗位的一次申请；同一人员可以有多个岗位申请，每个申请独立评分、独立决策、独立保留历史。
- 候选人页面按“HR 已通过的 Application”展示，不把 AI 完成评分等同于候选人通过；同一人员的不同岗位申请分别出现并独立处理，不建设按人汇总多个岗位申请的详情页。
...
- 阶段 6 已获用户最终确认；阶段 7 Application/状态和 Rubric 业务门禁继续有效。步骤 9，以及步骤 10 的候选人列表、`hr_direct` 新增、首次评分、跨岗位输入隔离、当前 Rubric 只读展示、模板/AI 草稿起草、既有语义项编辑保存、语义项新增/删除、五维权重受限调整、单个 HR 手动评分项 AI 辅助和 AI 优化当前占比已经完成；Candidate 详情聚合已取消。下一小步只接“放弃草稿并保留当前正式版本”，不提前进入发布、重新确认或阶段 8。
...
#### 小步骤 9 第七小段：强制重跑确认交互（已完成）

- 最新成功评分不再只显示禁用的“结果已最新”，而是提供独立“强制重跑”入口；普通首次评分、失败重试、过期更新和 blocked 禁用边界保持不变。点击强制重跑先展示候选人、岗位、当前分数与 attempt，并明确本次会真正调用 DeepSeek、产生新的模型耗时/费用、创建不可变评分记录、失败不覆盖旧成功结果且不会自动修改 HR 决策。
- 弹窗要求填写 1—1000 字重跑原因，纯空白不能提交；前端规范化首尾空格后复用既有 `POST /applications/{id}/screenings`，严格发送 `force=true`、`confirm_force=true` 和原因。提交期间复用单人同步守卫，禁用关闭、取消和重复确认；请求完成后刷新队列，成功提示旧结果仍保留，API 失败则保留弹窗以便 HR 查看原因后重试。
- `stage7-single-screening-ui.test.mjs` 新增最新结果动作、空白原因、强制载荷、弹窗关键文案和 560px 窄屏布局保护。定向单人评分测试、Application Service 测试和 `tsc && vite build` 通过；生产构建转换 3118 个模块。随后运行全部 15 组前端测试，全部通过，`git diff --check` 通过。曾尝试复跑后端 `ScreeningRunRequest` 与 `ScreeningService` 定向测试，但仓库根 `.venv` 和系统 Python 均未安装 pytest，因此测试未执行；本轮没有擅自安装或升级依赖，后端合同以既有测试记录和未修改的后端代码为边界。
- 浏览器验收边界：本轮把 Vite 临时启动到不冲突的 `127.0.0.1:5174` 并确认 HTTP 200，但浏览器运行时没有任何可用实例，无法完成真实点击、1440/390 截图或真实阶段 7 数据验证；临时 Vite 已精确停止。本轮没有连接正式 PostgreSQL、没有真实调用 DeepSeek，也没有执行一次真实强制重跑，因此自动化只能证明前端状态策略、载荷接线、重复提交保护、响应式 CSS 和生产编译，不能证明真实模型费用、数据库历史写入或最终视觉效果。
- 本段位于 `React 确认弹窗 -> applications.ts -> POST /applications/{id}/screenings -> 既有 ScreeningRunRequest -> ScreeningService -> ScreeningResult Model -> PostgreSQL/DeepSeek`。本次只修改 React、前端动作策略、样式和前端测试，没有修改 API、Schema、后端 Service、Model、Alembic 或 PostgreSQL。下一小步只做 HR 通过/备选/淘汰决策交互，不提前进入候选人页面聚合、岗位 Rubric 前端或阶段 8。

#### 小步骤 9 第八小段：HR 通过、备选、淘汰决策交互（已完成）

- `/app/screening` 每条 Application 新增单一“HR 决策”入口；没有在行内堆叠三个高风险按钮，而是使用独立决策单展示候选人、岗位和“当前状态 -> 目标状态”。`pending` 可选择通过/备选/淘汰，`passed` 可改为备选/淘汰，`backup` 可改为通过/淘汰，`rejected` 只能先撤销淘汰回到待审核；AI `screening`、无效或作废申请均禁止决策。
- 通过会根据当前成功且未过期的推荐等级、硬性条件和至少 60% 证据覆盖率自动区分 `meets_requirements` 与 `manual_override`；人工覆盖必须写岗位相关说明。备选和淘汰必须从固定业务原因中选择；任何已有决定的反转必须写说明；淘汰固定提交 `confirmed=true` 并明确只结束当前岗位申请，不删除 Candidate、Application、简历、评分或历史。撤销淘汰必须选择反转原因并填写说明，只回到 `pending/hr_review`，不自动通过或重新评分。
- 新增 `screeningDecisionAction.ts` 纯策略层，统一允许的状态迁移、原因选项、请求载荷、公平性即时校验和错误指引；React 复用既有 `pass/backup/reject/undo-rejection` API，使用同步 guard 防止重复提交，并让单人评分、批量评分和 HR 决策互斥。成功后刷新工作队列，失败保留决策单和后端错误指引。
- 后端 `stage_history.py` 将决策说明直接绑定到现有 Rubric 公平性禁止词校验，年龄、性别、民族、婚姻、生育、照片、籍贯、985/211、学校声誉等内容即使绕过前端也会被 Schema 拒绝；没有增加数据库字段、迁移或新的原因码。新增后端 Schema 测试覆盖通过、备选、淘汰、撤销与作废说明的相同边界。
- 验证结果：新增 `stage7-hr-decision-ui.test.mjs`；全部 16 组前端测试通过，`tsc && vite build` 成功转换 3119 个模块；后端 Schema/Service 19 项和 API 6 项 `unittest` 共 25 项通过，`compileall`、公平性 Unicode 冒烟验证和 `git diff --check` 通过。测试能证明状态策略、严格载荷、同步互斥、API 映射、事务回滚和公平性 Schema，但本轮没有连接真实 PostgreSQL 执行一次决策写入。
- `frontend-design` 技能把界面收敛为一个可聚焦的决策单，用绿色通过、琥珀备选、红色淘汰表达业务去向，保留键盘 focus 样式并在 560px 以下改为单列。浏览器运行时仍没有任何可用实例，因此没有完成真实点击、1440/390 截图或最终视觉验收；不能用 CSS 检查和生产构建替代这些证据。
- 本段位于 `React 决策单 -> applications.ts -> /applications/{id}/pass|backup|reject|undo-rejection -> StageHistory Schema -> ApplicationDecisionService -> Application/StageHistory/ActivityLog Model -> PostgreSQL`。本次修改前端、决策说明 Schema 和测试，没有修改 API 路径、Service 状态机、Model、Alembic 或 PostgreSQL。下一小步只做 AI 初筛中心“录入新申请”前端入口，不提前进入候选人页面聚合、岗位 Rubric 前端或阶段 8。

#### 小步骤 9 第九小段：AI 初筛中心录入新申请（已完成）

- `/app/screening` 页面标题区新增主入口“录入新申请”，使用独立任务单展示“保存申请 -> AI 初筛 -> HR 决策”三段业务轨道，明确该入口不是 HR 人工直通。表单只收候选人姓名、有效手机、有效邮箱、开放岗位和本次简历；请求固定为 `source=hr_screening`、`confirm_hr_pass=false`，因此新申请保持 `pending`，不会绕过 HR 决策。
- 简历支持两种来源：上传 PDF/DOCX/TXT 后调用既有上传与原文提取 API，或从已有 `parsed` 简历中选择。新上传文件在取消、切换来源或替换文件时尝试调用既有未绑定 Resume 清理接口；如果清理失败会明确提示 Resume ID，不静默遗留。关闭岗位不出现在选择项中，现有简历加载失败可单独重试。
- 提交流程严格先调用 `POST /api/v2/applications/intake` 保存 Candidate/Application，再对新建 Application 调用既有单人评分接口。AI `failed/blocked` 或启动失败只显示告警并保留已保存申请；同 Candidate/Job 已有 active Application 时复用现有记录，不重复创建、评分或产生模型费用。联系方式身份冲突、简历归属冲突、关闭岗位和简历失效继续使用后端稳定业务错误，并显示候选人冲突 ID 供 HR 核对。
- 新增纯前端策略 `screeningIntakeAction.ts`，集中校验姓名、7—15 位手机号、邮箱、岗位、简历、支持格式和稳定错误文案；新增 `stage7-screening-intake-ui.test.mjs` 验证固定请求语义、输入边界、调用顺序、错误映射、响应式样式和 reduced-motion。全部 17 组前端测试、31 项既有 intake Schema/Service/API 定向测试和 25 项 HR 决策后端定向测试通过，`compileall` 与 `tsc && vite build` 通过，生产构建转换 3120 个模块。
- `frontend-design` 技能把新入口收敛成一个任务单，沿用现有深蓝工作台视觉，并用三段轨道作为唯一强调点；560px 以下表单和流程轨道改为单列，上传区域保留键盘焦点反馈，系统关闭动画偏好时停止进度图标动画。浏览器插件运行时没有任何可用实例，因此未完成真实点击、1440/390 截图和最终视觉验收。
- 本段位于 `React 录入任务单 -> resumes.ts/applications.ts -> Resume 上传与原文提取 API + POST /applications/intake + POST /applications/{id}/screenings -> 既有 Schema -> ApplicationIntakeService/ScreeningService -> Candidate/Resume/Application/ScreeningResult Model -> PostgreSQL/DeepSeek`。本次只修改 React、前端策略/测试/样式和权威文档，没有修改后端 API、Schema、Service、Model、Alembic 或 PostgreSQL；自动化证明请求顺序、边界和生产构建成立，但没有证明真实 PostgreSQL 写入或真实 DeepSeek 调用。下一小步只改造候选人页面只展示 HR 已通过的 Application。

#### 小步骤 10 第一小段：候选人列表按已通过 Application 展示（已完成）

- 候选人列表的数据源从“全量 Candidate + 旧 `applied_job_id`”改为“`hr_decision=passed` Application + Candidate + Job + 当前 ScreeningResult”。Candidate 只保存稳定人员资料，是否进入候选人业务视图由岗位级 Application 决策决定，不再通过 Candidate 是否存在来判断。
- 每一行对应一个 Application，并使用 `applicationId` 作为 React 唯一键；同一 Candidate 通过两个岗位时展示两行，但统计中的“实际人数”只计一次。岗位筛选只列出当前通过申请涉及的岗位，关闭岗位的既有通过记录仍可查看，不会因为岗位关闭丢失历史。
- 列表展示候选人、已通过岗位、当前 AI 分数/推荐等级、AI 状态、招聘阶段、申请来源和 Application 更新时间。`hr_direct` 记录即使 AI 为 not_started/screening/failed/blocked 仍显示；评分失败、缺少当前结果或结果过期计入“评分待跟进”，但不会撤销 HR 已通过归类。
- 页面新增深绿色“准入规则条”，明确“HR 已通过 Application -> 进入候选人业务视图”；待决策、备选和淘汰留在初筛中心。旧“新增候选人”只创建裸 Candidate，暂时禁用并说明下一小段升级原因，避免继续写入无法进入新列表的半成品数据。
- 新增 `stage7-candidate-application-list.test.mjs`，验证请求固定带 `hr_decision=passed`、同一 Candidate 多岗位分行、HR 直通失败评分仍保留、备选不出现、统计与排序、页面文案和 560px 响应式规则；同步更新关闭岗位读取边界测试。全部 18 组前端测试与 `tsc && vite build` 通过，生产构建转换 3120 个模块。
- `frontend-design` 技能让准入关系成为页面唯一视觉强调点，避免继续用普通状态标签掩盖 Candidate/Application 分离。当前浏览器运行时没有可用实例，因此未完成真实点击、真实数据渲染和 1440/390 截图验收。
- 本段位于 `React 候选人列表 -> candidates.ts/listStage7Applications -> GET /applications?hr_decision=passed + candidates/jobs/screening-results -> 既有 Schema/Service -> Application/Candidate/Job/ScreeningResult Model -> PostgreSQL`。本次只修改前端聚合 Service、React、CSS、测试和文档，没有修改后端 API、Schema、Service、Model、Alembic 或 PostgreSQL；自动化证明过滤、映射和构建成立，但没有证明正式数据库真实列表。下一小步只升级 HR 直接新增入口与首次自动评分。

#### 小步骤 10 第二小段第一部分：HR 直接新增与首次自动评分（已完成）

- 候选人页面的“新增候选人”已从禁用的旧裸 Candidate 入口升级为可用的 `hr_direct` Application 入口。表单现在强制姓名、有效手机、有效邮箱、开放岗位、当前简历，并要求 HR 勾选“我已核对岗位和候选人资料，并确认 HR 人工通过”；页面明确 AI 只提供岗位匹配参考，低分、资料不足或执行失败都不能撤销人工通过。
- `ApplicationIntakeRequest` 新增可选 `candidate_profile`，只接收性别、年龄、城市、当前经历、工作年限、学历、来源、技能及教育/工作/项目经历，不允许嵌套 profile 改写身份、岗位、状态和简历系统字段。新 Candidate 使用复用后的 `CandidateService.build_candidate()` 组装，使完整资料、Resume 绑定、Application 和首条 StageHistory 共用一个数据库事务，任一步失败全部回滚。
- 联系方式共同命中已有 Candidate 时仍安全复用，本次 profile 不会静默覆盖旧正式资料，前端会明确提示 Candidate ID；如果同岗位已有待决策 active Application，明确的 `hr_direct + confirm_hr_pass=true` 会复用该 Application，在同一事务转为 `passed/screening_passed` 并追加 `hr_direct_entry` 历史。本段结束时曾计划增加跨简历资料差异合并，随后已被用户确认的新业务规则取消。
- 前端严格先调用 intake；只有返回 Application 的 `ai_status=not_started` 才自动调用单人评分，已有评分状态不重复产生模型调用。评分 completed/failed/blocked/启动异常都有独立反馈；评分接口异常时申请与人工通过已经安全保存，HR 可在 AI 初筛中心重试。
- `frontend-design` 技能把绿色“人工通过”凭据作为确认卡的唯一强调点，继续沿用现有工作台色彩、排版和响应式结构，没有重做简历识别表单。新增 `stage7-hr-direct-candidate-ui.test.mjs`，并同步更新旧“待升级”断言。
- 验证结果：Application intake Schema/Service/API 定向 35 项通过，后端全量 600 项通过；全部 19 组前端测试通过，`tsc && vite build` 成功转换 3120 个模块，`compileall` 通过。Docker Desktop 当前未运行且浏览器运行时仍无可用实例，因此没有执行真实 PostgreSQL 写入、真实 DeepSeek 调用、真实点击或 1440/390 截图；自动化不能证明这些外部运行结果。
- 本段位于 `React HR 人工确认 -> candidates.ts/applications.ts -> POST /applications/intake -> ApplicationIntakeRequest -> ApplicationIntakeService/CandidateService -> Candidate/Resume/Application/StageHistory Model -> PostgreSQL`，评分续接为 `POST /applications/{id}/screenings -> ScreeningService -> ScreeningResult -> PostgreSQL/DeepSeek`。本次修改前端、Schema、Service 与测试，没有修改 API 路径、Model、Alembic 或正式 PostgreSQL。后续范围以紧随其后的需求修订检查点为准。

#### 小步骤 10 需求修订检查点：不同岗位简历独立（已确认）

- 用户明确确认：同一个人投递不同岗位时使用内容不同的简历是正常业务行为，不需要展示新旧职业资料差异，也不需要把经历、技能、学历和项目合并成 Candidate 全局画像。
- 新边界为：Candidate 只负责姓名、标准化联系方式等稳定身份与去重；每个 Application 独立绑定自己的当前 Resume；Python 硬规则、DeepSeek 语义评分、输入快照和指纹都必须只读取当前 Application 的 Resume 材料。只有手机/邮箱身份指向冲突、Resume 已属于其他 Candidate 或同 Candidate/Job active Application 重复时才需要人工核对。
- 只读代码审计确认数据库关系方向正确：`Application.current_resume_id` 已把每次申请与当前 Resume 分开，ScreeningResult 也保存 `application_id/resume_id`。但当前 `ScreeningService._candidate_profile()` 会把 Candidate 的 `parsed_data`、工作/项目/教育记录组装成 `confirmed_profile`，确定性年限、学历和技能规则优先读取该全局 profile，因此第二个岗位存在被第一个岗位材料污染的风险。
- 当前 `applications/intake.candidate_profile` 仍会在新建 Candidate 时写入全局职业资料；该实现与新确认边界不完全一致。下一小步需要把它改为当前 Resume 的 HR 确认材料，并同步修正评分输入、快照、指纹及跨岗位隔离测试。此检查点只更新设计与状态文档，没有修改业务代码、Model、Alembic 或数据库。
- 本次需求修订位于 `Candidate 身份 -> Application 岗位事实 -> current Resume 岗位材料 -> ScreeningResult` 的数据归属边界。后续界面是否按人聚合在下一检查点重新确认。

#### 小步骤 10 第二小段第二部分：不同岗位简历评分隔离（已完成）

- intake 合同将 `candidate_profile` 更名为 `resume_profile`；经历、学历、技能、项目和 HR 确认字段写入当前 `Resume.parsed_snapshot.confirmed_profile`。新 Candidate 只保存姓名、标准化手机/邮箱、来源和初始状态，不再复制 Resume 原文、结构化草稿、岗位或职业经历；同 Candidate/Job active Application 复用路径不会改写未采用的 Resume profile。
- `ScreeningService` 删除 Candidate 全局职业画像组装和相关经历预加载，评分材料只从 `Application.current_resume` 的原文、结构化草稿和 `confirmed_profile` 构造。HR 确认资料存在时供确定性规则优先使用；不存在时回退到当前 Resume 的 AI 结构化草稿。模型输入、候选人快照、Resume 快照和输入指纹均由这份隔离后的材料生成。
- 新增跨岗位隔离回归：即使把同一 Candidate 的全局工作年限改为 99、学历改为博士，并写入另一岗位技能，当前 Application 的材料和输入指纹仍保持不变；同时覆盖 HR screening 无确认资料时从当前 Resume 草稿读取年限、学历和技能。`ResumeStructureSnapshot` 的严格 Schema 已允许可选 `confirmed_profile`，既有快照仍可读取；同一 Resume 强制重新识别时只替换 AI 草稿和元数据，保留该 Resume 自己的 HR 确认资料。
- 验证结果：相关 Schema/Service 54 项通过，后端全量 601 项通过；前端 19 组测试全部通过，`tsc && vite build` 成功转换 3120 个模块。真实 PostgreSQL 使用外层事务回滚完成 intake 验证，确认 Resume JSONB 持久化 HR profile、Candidate 全局职业字段保持为空且 HR 直通 Application 为 passed；测试数据已回滚，没有留下业务记录。未调用真实 DeepSeek，也未执行浏览器人工验收。
- 本段位于 `React HR 简历确认 -> applications/intake.resume_profile -> ApplicationResumeProfile Schema -> ApplicationIntakeService -> Candidate 身份 + Resume 岗位材料 + Application -> PostgreSQL JSONB`，评分链路为 `Application.current_resume -> ScreeningInputService/ScreeningService -> Python 规则 + DeepSeek Adapter -> ScreeningResult -> PostgreSQL`。本次修改 Schema、Service、前端请求类型/文案、测试和权威文档，没有修改 API 路径、Model、Alembic 或数据库表结构。后续界面范围以紧随其后的产品决策为准。

#### 小步骤 10 产品决策：取消跨岗位 Candidate 详情聚合（已确认）

- 用户于 2026-08-19 确认：HR 没有必要查看“同一个人投递过的全部岗位”的聚合详情，也不需要在界面上整合、比较或关联展示不同岗位申请。候选人列表和后续业务操作继续以 Application 为一条独立记录。
- Candidate 仍在数据库中承担姓名、标准化手机/邮箱等稳定身份与去重，避免重复人数、同岗位重复申请和隐私数据处理失控；该后台身份复用不要求前端向 HR 展示跨岗位关系。
- 同一 Candidate 的两个岗位申请在界面上可以像两条互不相关的候选记录：每条只展示自己的 Job、Resume、ScreeningResult、HR 决策和招聘阶段。需要查看证据或历史时进入该 Application/评分结果的岗位级视图，不进入按人聚合页面。
- 本检查点只修改产品范围和权威文档，没有修改前端、API、Schema、Service、Model、Alembic 或 PostgreSQL。原计划的 Candidate 详情聚合从阶段 7 删除，下一小步直接进入岗位 Rubric 前端编辑和版本提示。

#### 小步骤 10 第三部分：岗位当前 Rubric 只读展示（已完成）

- 岗位列表新增“评分规则”入口，通过现有 `GET /api/v2/jobs/{job_id}/screening-rubric` 读取当前正式版本。前端新增独立 TypeScript 合同和 snake_case 到 camelCase 映射，并覆盖 loading、失败重试、空语义项和 stale 待重新确认状态。快速切换岗位时使用请求序号防止较慢的旧响应覆盖新数据。
- 只读抽屉展示 `vN`、生效/过期状态、来源与模板、五维权重轨道、4—10 个语义项的维度、建议占比、说明和高/中/低分锚点，同时明示“Python 比较硬条件，DeepSeek 只评价语义项”的责任边界。“编辑评分规则”保留为明确禁用的下一步入口，本步没有伪造未实现的编辑、AI 生成或发布操作。
- 新增 1 组 Rubric 前端合同/UI 测试；前端全量 20 组测试通过，`tsc && vite build` 成功转换 3122 个模块。曾精确启动 `127.0.0.1:5175` 尝试视觉验收，但当前会话没有可用浏览器实例，临时 Vite 已停止；因此不把响应式源码检查和生产构建冒充为桌面/窄屏视觉验收。
- 本段位于 `React 岗位页 -> jobRubrics.ts -> GET /jobs/{id}/screening-rubric -> JobScreeningRubricRead -> ScreeningRubricService -> JobScreeningRubric -> PostgreSQL`。本步只修改前端和文档，没有修改后端 API、Schema、Service、Model、Alembic 或 PostgreSQL。下一小步只接入草稿查询和 standard/technical/non_technical 三套模板起草，暂不接 AI 生成、单项辅助或发布。

#### 小步骤 10 第四部分：Rubric 草稿查询与模板起草（已完成）

- “编辑评分规则”入口已从禁用占位改为真实草稿流程：先调用 `GET /api/v2/jobs/{job_id}/screening-rubric/draft`。命中草稿时展示未发布版本、模板来源、五维权重、变更说明和语义项概览；只有稳定 `RUBRIC_DRAFT_NOT_FOUND` 才进入新建界面，其他错误保留重试。
- 无草稿时 HR 可在 standard、technical 和 non_technical 三套后端版本化模板中选择一套，并填写进入审计记录的起草原因。前端通过 `POST /api/v2/jobs/{job_id}/screening-rubric/draft/from-template` 固定提交 `replace_existing=false`，不允许这一小步静默替换他人或早先的草稿。并发返回 `RUBRIC_DRAFT_ALREADY_EXISTS` 时自动收口为读取已存在草稿，当前 active Rubric 始终不变。
- 界面使用深蓝表示正式版本、琥珀表示未发布草稿，在模板选择页反复明示“候选人评分仍使用当前正式版本”。已有草稿只显示摘要，“继续编辑评分项”保留为下一小步的禁用入口；本步没有接入逐项编辑、AI 生成、AI 单项辅助、放弃或发布。
- 新增 1 组草稿 Service/UI 测试；最终前端全量 21 组测试通过，`tsc && vite build` 成功转换 3123 个模块。测试证明请求路径、载荷、响应映射、404/409 分支、页面必要文案和现有前端回归稳定；未连接真实 PostgreSQL 创建草稿，当前会话仍没有可用浏览器实例，因此不能证明真实写入、最终桌面/窄屏视觉和真实点击。
- 本段位于 `React 草稿界面 -> jobRubrics.ts -> GET/POST Rubric draft API -> ScreeningRubricTemplateDraftRequest/JobScreeningRubricRead -> ScreeningRubricService -> JobScreeningRubric/ActivityLog -> PostgreSQL`。本步复用已有后端合同，没有修改 API、Schema、Service、Model、Alembic 或 PostgreSQL。下一小步只做草稿语义评分项的逐项编辑和保存，暂不接 AI 生成、单项辅助或发布。

#### 小步骤 10 第五部分：Rubric 草稿既有语义项编辑与保存（已完成）

- 草稿摘要的“继续编辑评分项”已进入真实表单。HR 可逐项修改名称、固定五维归属、建议占比、可从简历核对的说明及高/中/低分锚点；稳定 key、来源和固定 `max_score=10` 不在界面开放修改，不会因前端重建表单而丢失。
- 新增 `PUT /api/v2/jobs/{job_id}/screening-rubric/draft` 前端映射，仅提交 `expected_job_fingerprint + semantic_items + change_detail`，不夹带五维权重。岗位指纹用于防止岗位评分输入变化后仍覆盖旧草稿；后端继续负责重复项、公平性禁止内容、字段范围和指纹一致性的最终校验。FastAPI 422 字段错误也已转换为不泄露内部结构的前端提示。
- 编辑器要求填写本次修改说明；返回正式版本、点击抽屉关闭、点击遮罩或表单内取消时，未保存修改都会二次确认。保存期间禁止关闭、键盘退出和返回，成功后回到最新草稿摘要并明示正式版本未改变。
- 新增 1 组语义项编辑器 Service/UI 测试；最终前端全量 22 组测试通过，`tsc && vite build` 成功转换 3124 个模块。证据覆盖 PUT 路径和 snake_case 载荷、岗位指纹、旧字段保留、422 错误映射、未保存保护、保存期间关闭保护、响应式 CSS 和现有前端回归；未连接真实 PostgreSQL 执行 PUT，也没有可用浏览器实例完成真实点击和视觉验收。
- 本段位于 `React 语义项编辑器 -> jobRubrics.ts -> PUT Rubric draft API -> ScreeningRubricDraftUpdateRequest/JobScreeningRubricRead -> ScreeningRubricService -> JobScreeningRubric/ActivityLog -> PostgreSQL`。本步复用已有后端合同，没有修改 API、Schema、Service、Model、Alembic 或 PostgreSQL。下一小步只做语义评分项的新增/删除与 4—10 条数量保护，暂不接五维权重、AI 生成、单项辅助或发布。

#### 小步骤 10 第六部分：Rubric 草稿语义项新增/删除（已完成）

- 既有编辑器现在支持从零新增 `hr_manual` 语义评分项和删除不适用项。新增项由前端自动生成符合后端合同的稳定 `hr_manual_...` key，固定 `max_score=10`、来源为 `hr_manual`，HR 只需填写名称、五维归属、建议占比、说明和高/中/低分锚点；增删后的完整列表继续通过同一个带岗位指纹的 PUT 保存。
- 编辑器把 4—10 条总量变成可见操作边界：第 4 项不能继续删除，第 10 项不能继续新增，计数与剩余名额实时展示；删除先二次确认且只改变未保存表单，点击“保存草稿修改”后才写入。保存前还有 4—10 项防御性检查，后端继续负责最多 10 项、唯一 key、名称去重、公平性校验，并在正式发布时强制不少于 4 项。
- 视觉上继续把每条语义项表现为琥珀色评分尺卡片，只在卡片头部增加克制的删除动作，并在列表末尾用虚线区域承接新增入口；560px 窄屏下卡片头部和操作区改为纵向布局。`frontend-design` 技能用于保持正式版本深蓝、草稿琥珀的现有语义，没有引入拖拽或额外动画。
- 扩展原语义项编辑器测试，覆盖 4/10 常量、手动项 key/来源/固定满分、增删文案与按钮边界、增删后不再依赖原数组下标保存，以及响应式样式。前端全量 22 组测试通过，`tsc && vite build` 成功转换 3124 个模块；未连接真实 PostgreSQL 执行增删 PUT，当前会话没有可用浏览器实例，因此不能证明真实点击、最终视觉或数据库持久化。
- 本段位于 `React Form.List 增删 -> RecruitmentJobRubricItemEditor -> jobRubrics.ts -> PUT Rubric draft API -> ScreeningRubricDraftUpdateRequest/JobScreeningRubricRead -> ScreeningRubricService -> JobScreeningRubric/ActivityLog -> PostgreSQL`。本步只修改前端、测试和权威文档，没有修改后端 API、Schema、Service、Model、Alembic 或 PostgreSQL。下一小步只做五维权重的受限调整与总和 100 保护，暂不接 AI 生成、单项辅助或发布。

#### 小步骤 10 第七部分：Rubric 草稿五维权重调整（已完成）

- 语义项编辑器顶部新增“100 分配额账本”，HR 可调整必备条件、经历职责、项目深度、加分项和补充要求五个大维度；每项分别限制在后端既有的 `30—50 / 15—35 / 10—30 / 0—20 / 0—10` 整数范围内。界面明确区分“大维度总权重”和语义项在所属维度内部的“建议占比”，避免把两层比例混为一谈。
- 当前合计实时显示为 `N / 100`，低于 100 时提示还需分配多少，超过 100 时提示超出多少；只有恰好等于 100 才启用保存。页面提供“恢复默认 40/25/20/10/5”，该动作只改当前未保存表单，仍需填写修改说明并通过岗位指纹 PUT 保存，不会直接改动正式版本。
- `jobRubrics.ts` 的草稿更新类型改为“权重或语义项至少提交一种”的 TypeScript 联合类型，并支持把 camelCase 权重映射为后端 snake_case；编辑器保存时同时提交当前权重和完整语义项。后端既有 `ScreeningRubricWeights` 继续负责整数、单项范围和总和 100 的最终校验，本步未放宽任何合同。
- `frontend-design` 技能让权重区域采用正式的配额账本而不是普通设置卡片：深蓝承载规则信息，合法合计使用绿色，缺失/超额使用红色，实时进度尺是唯一强调点；820px 与 560px 下分别收为三列和两列，并为减少动态效果偏好关闭进度过渡。
- 扩展语义项编辑器测试，新增权重 snake_case 请求载荷、可选更新内容、默认权重合计、必要文案、非 100 禁止保存、五列响应式结构和 reduced-motion 断言。前端全量 22 组测试通过，`tsc && vite build` 成功转换 3124 个模块；未连接真实 PostgreSQL 执行权重 PUT，也没有可用浏览器实例完成真实输入、点击与视觉验收。
- 本段位于 `React 五维配额账本 -> RecruitmentJobRubricItemEditor -> jobRubrics.ts -> PUT Rubric draft API -> ScreeningRubricDraftUpdateRequest/ScreeningRubricWeights -> ScreeningRubricService -> JobScreeningRubric/ActivityLog -> PostgreSQL`。本步只修改前端、测试和权威文档，没有修改后端 API、Schema、Service、Model、Alembic 或 PostgreSQL。下一小步只接 AI 根据岗位生成整份 Rubric 草稿及已有草稿替换确认，暂不接单项辅助、放弃、发布或重新确认。

#### 小步骤 10 第八部分：AI 根据岗位生成整份 Rubric 草稿（已完成）

- 无草稿界面现在同时提供“按模板创建草稿”和“AI 根据岗位生成草稿”。HR 先选择 standard/technical/non_technical 生成方向并填写进入审计记录的原因；AI 请求固定使用 `POST /api/v2/jobs/{job_id}/screening-rubric/generate`，无草稿路径发送 `replace_existing=false`，成功后只进入未发布草稿摘要，当前正式版本继续参与候选人评分。
- 已有草稿摘要新增“让 AI 重新起草整份内容”。重新生成前必须选择方向、填写原因并通过危险操作二次确认，确认后才发送 `replace_existing=true`；界面明确说明旧草稿会保留为已放弃的审计记录，尚未发布的人工修改不会自动并入新草稿，正式版本不会改变。
- AI 生成期间，选项、文本框和重复提交均被锁定，pending 状态继续上送 Drawer，阻止遮罩关闭、键盘退出和返回正式版本。生成成功后显示新的草稿来源和成功提示；模型不可用、非法输出、岗位指纹变化或并发草稿冲突使用后端稳定错误，界面说明当前草稿与正式版本仍被保留。
- `frontend-design` 技能把 AI 能力表达为“岗位要求 -> 生成 5—8 个语义项 -> HR 继续编辑”的工作线，使用克制的靛蓝强调而不是通用发光按钮；820px 下工作线和方向选择收为单列，没有引入持续动画。
- 扩展草稿 Service/UI 测试，覆盖 generate 路径、模板方向、`replace_existing=true` 载荷、无草稿/已有草稿文案、替换确认、失败保留说明和响应式工作线。前端全量 22 组测试通过，`tsc && vite build` 成功转换 3124 个模块；本步未调用真实 DeepSeek、未连接真实 PostgreSQL 验证替换事务，也没有浏览器实例完成真实点击和视觉验收。
- 本段位于 `React AI 起草工作线 -> jobRubrics.ts -> POST Rubric generate API -> ScreeningRubricGenerateRequest -> ScreeningRubricService/RubricGenerationAdapter -> JobScreeningRubric/ActivityLog -> PostgreSQL/DeepSeek`。本步只修改前端、测试和权威文档，没有修改后端 API、Schema、Service、Adapter、Model、Alembic 或 PostgreSQL。下一小步只接 AI 辅助完善单个 HR 手动评分项，暂不接放弃、发布或重新确认。

#### 小步骤 10 第九部分：AI 辅助完善单个 HR 手动评分项（已完成）

- Rubric 编辑器只在 `source=hr_manual` 的手动评分项上显示“AI 辅助完善”。HR 必须先填写名称、固定维度、当前建议占比、评分说明及高/中/低分标准；前端再调用既有 `POST /api/v2/jobs/{job_id}/screening-rubric/draft/assist-item`，岗位指纹继续阻止岗位输入变化后的过期建议。
- AI 返回后先展示独立“单项校准单”，列出建议评分说明与 `9—10 / 4—8 / 0—3` 三档证据标准。HR 可以保留原内容或明确采用；采用只回填说明和三档锚点，不修改名称、所属维度或建议占比，不自动保存草稿，更不会发布正式版本。本部分没有把整份占比优化混入单项辅助。
- AI 调用期间阻止关闭 Rubric 抽屉和重复请求；必填内容不完整时不调用模型。模型不可用、非法输出、岗位指纹变化或网络失败只显示稳定错误，当前表单、草稿和正式版本均保持不变。单项建议的模型、Prompt 版本和 token 元数据只用于校准单说明，不写入未发布表单。
- `frontend-design` 技能把 AI 返回设计成四段式“校准尺”，延续草稿琥珀、AI 靛蓝和高/中/低证据绿/金/红的现有视觉语义；560px 下改为单列，未新增持续动画。
- 验证结果：单项 Service/UI 定向测试通过；前端全部 22 组测试通过；`tsc && vite build` 成功转换 3124 个模块；既有 Rubric API/生成 Service 16 项后端定向测试通过。当前没有可用浏览器实例，因此未完成真实点击和 1440/390 截图；本步也没有调用真实 DeepSeek 或连接真实 PostgreSQL 保存草稿。
- 本段位于 `React 手动项编辑器 -> jobRubrics.ts -> POST assist-item -> ScreeningRubricItemAssistRequest/Response -> ScreeningRubricService/RubricGenerationAdapter -> DeepSeek`。AI 结果只返回前端内存供 HR 预览和选择；真正持久化仍需已有的草稿 PUT。本步只修改前端、测试和权威文档，没有修改后端 API、Schema、Service、Adapter、Model、Alembic 或 PostgreSQL。

#### 小步骤 10 第十部分：AI 优化当前占比（已完成）

- Rubric 编辑器新增“AI 优化当前占比”。请求携带当前未保存表单中的五维总权重、4—10 个完整语义评分项、草稿 ID 和岗位指纹，因此本轮新增、删除或修改过的评分项也会参与建议；AI 只返回每个既有 `key` 的 `suggested_share + reason`，不修改五维总权重、Python 确定性规则、名称、维度、说明或评分锚点。
- 新增 `POST /api/v2/jobs/{job_id}/screening-rubric/draft/optimize-shares`。Pydantic 请求/响应合同限制 4—10 项、占比 1—100、禁止重复 key；Service 在模型调用前后复查岗位指纹与草稿 ID，并要求返回 key 集合与请求完全一致。缺项、增项、重复项、非法 JSON、岗位变化或草稿被替换都只返回稳定错误，接口全程只读，不执行 commit。
- AI 返回后打开“AI 占比校准台”，逐项展示当前值、建议值和调整理由。HR 可以保留当前值或明确应用；应用严格按稳定 key 只回填 `suggestedShare`。如果模型等待期间表单内容已变化，旧建议会被拒绝；应用后仍只是未保存表单，必须再执行既有草稿 PUT 才会进入 PostgreSQL，更不会自动发布正式版本。
- `frontend-design` 技能把整份占比建议设计为紧凑的对比台账，延续现有 AI 靛蓝语义，清晰区分旧值、箭头、新值和理由；窄屏下收为两列加理由单独一行，没有引入持续动画。
- 新增/扩展 Schema、Prompt、Service、API 和前端 Service/UI 回归，覆盖固定五维权重不变、请求映射、建议映射、完全一致的 key 集合、只读事务、过期表单保护和响应式布局。`python -m compileall`、后端全量 604 项、前端全量 22 组测试均通过；`tsc && vite build` 成功转换 3124 个模块，`git diff --check` 无空白错误。本步未调用真实 DeepSeek、未连接真实 PostgreSQL，也没有可用浏览器实例完成真实点击和截图验收。
- 本段位于 `React 占比校准台 -> jobRubrics.ts -> POST optimize-shares -> ScreeningRubricShareOptimizationRequest/Response -> ScreeningRubricService -> RubricGenerationAdapter -> DeepSeek`。Model、Alembic 与 PostgreSQL Schema 不变；只有 HR 后续点击“保存草稿修改”时，既有 `PUT draft` 才负责持久化。下一小步只接“放弃草稿并保留当前正式版本”，暂不接发布或岗位变化后的重新确认。
