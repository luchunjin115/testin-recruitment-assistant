...
用户已继续确认小步骤 5A 的剩余规则：确定性规则和语义项共同分配所属维度的内部占比，系统提供安全初值，AI 可以建议生成或优化占比但只能写入草稿；无任何规则或评分项的维度视为不适用并按原比例把权重分配给其他有效维度，已配置但候选人缺少证据则保持 `unknown`；影响评分的岗位字段变化会使当前 Rubric 待重新确认并暂停新评分；AI 失败或岗位在生成期间变化不得破坏当前正式版本；每个岗位最多一个编辑草稿，发布生成不可变新版本，放弃和替换均留审计；HR 可以让 AI 生成整份评分标准或辅助完善单个手动项，所有结果仍须后端校验和 HR 发布。
...
确定性规则与语义评分项共同分配所属大维度内部的 100% 占比。模板项使用模板版本内的预设占比，AI 生成或优化时可以返回建议占比，HR 手动项由 HR 填写建议占比；缺少可用建议时，Service 对该维度内的有效项使用等额占比作为确定性安全初值。所有来源最终都由 Service 归一化并校验为该维度内部合计 100%，AI 不能修改五个大维度本身的权重、删除确定性规则或绕过 HR 发布。

某个大维度没有任何确定性规则和已发布语义项时视为“不适用”，其大维度权重按其他有效大维度的原始权重比例重新分配，最终有效总权重仍为 100%。岗位已经配置评分内容但候选人缺少证据时必须返回 `unknown` 并降低证据覆盖率，不能把该项或该维度当作不适用后重新分配。
...
每个岗位同时最多存在一个可编辑草稿和一个当前 active 版本。active 版本不可原地修改；开始编辑时从当前版本、预设模板、AI 生成结果或空白结构创建草稿。已有草稿时再次生成必须先二次确认，旧草稿标记为已放弃并保留审计，新生成内容不得静默覆盖人工修改。

发布时后端必须重新校验语义项数量、维度内部占比、评分锚点、确定性规则重复、公平性禁止项和岗位输入指纹。校验通过后草稿成为新的不可变 active 版本，旧 active 版本转为历史；旧评分结果继续关联原版本并标记 outdated，不自动重跑。放弃草稿不影响当前 active 版本；如果当前版本因岗位变化已待重新确认，放弃草稿后仍暂停新评分。

HR 不能通过普通表单删除系统确定性规则、突破五维权重范围、把总权重改为非 100、加入公平性禁止项或直接编辑底层系统 Prompt。发布后若已有评分结果，旧结果保留并标记 outdated，不自动重跑、不改变 HR 决策。

HR 手动新增语义项时只填写名称、固定五维归属、可从简历核对的说明、同维度建议占比以及高/中/低分锚点；系统自动生成稳定 key，`max_score` 第一版固定为 10。页面可以调用 AI 辅助完善单个手动项的说明和锚点，但结果仍只进入草稿，并接受与整份 AI 生成 Rubric 相同的后端校验和发布流程。

### 12.5 岗位变更、生成失败与过期

只有参与评分的岗位字段进入岗位指纹，包括标题、职责、必备/加分技能、最低年限、学历、必备/加分经历、关键词、补充要求和明确标记参与评分的岗位描述。招聘人数、状态等不参与评分的管理字段变化不使 Rubric 过期。

参与评分字段变化后，当前 active Rubric 标记为待重新确认，既有结果保留并标记 outdated，新申请评分暂停。HR 可以重新生成、手动修改，或明确确认原 Rubric 仍适用；无论内容是否变化，重新确认都发布一个绑定新岗位指纹的新版本，不能原地改写旧版本。

AI 超时、服务失败、非法数量、非法 Schema、重复计分、公平性禁止项或生成期间岗位指纹变化时，本次生成整体失败或成为不可发布的过期草稿，当前 active Rubric 和历史结果不变。页面提供重试、使用预设模板、手动创建和重新确认原版本入口；失败只保存脱敏后的稳定错误、模型、耗时和必要审计信息。
...
### 小步骤 5A：Reqcore 式 Rubric 方案调整（已完成）

- 用户已确认 AI 默认生成 5—8 个语义项、Schema 允许 4—10 个、Python 确定性规则不计入该数量，以及“结构化 JobRequirements 编译为 Python 规则、HR 手动语义项发布后注入 DeepSeek”的双入口；第 12 节的内部占比、AI 权重建议、空维度、岗位变更、失败语义、单草稿发布流程、AI 辅助手动项和验收场景也已确认，可以进入业务代码。
...
小步骤 5A 至少覆盖以下验收场景：技术和非技术岗位生成 5—8 个合理评分项；HR 手动新增并用 AI 完善单项标准；重复项、确定性规则重复计分和公平性禁止项被拒绝；草稿不能用于评分；编辑、放弃、替换和发布语义正确；新版本发布后旧版本保留；岗位变化使 Rubric 与旧结果过期并暂停新评分；AI 超时或非法输出不影响当前正式版本；生成期间岗位变化的草稿不能发布。完整 Fake 测试通过后，才使用少量脱敏岗位调用真实 DeepSeek。

完成进度：✅ 已完成。严格语义项、三套模板、两类 Prompt Builder、Rubric v2 Model/migration、单草稿生命周期、岗位 stale/outdated 事务，以及 RubricGenerationAdapter/Fake AI 整份生成与单项辅助闭环均已实现。真实 PostgreSQL 完成 `c8 -> e7 -> c8 -> e7` 和无漂移验证，并据此修复 JSON bind migration 缺陷；真实 DeepSeek 在 Prompt v2 下完成技术岗位 6 项、非技术岗位 7 项和单项辅助验证。后端全量 545 项测试通过，正式开发库未升级，前端将在后续 UI 步骤接入。
