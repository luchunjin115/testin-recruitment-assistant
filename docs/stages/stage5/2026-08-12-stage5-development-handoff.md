...
5. ✅ 结构化 API
6. ✅ 新增候选人页面辅助填表（前端类型/API、识别状态、普通字段安全合并、三类经历与技能人工确认导入、未建模内容安全展示已完成）
...
下一小步只处理技能候选以及证书、自我评价、warnings 等暂未建模内容的安全展示；`missing_fields` 保留在草稿中用于质量评估但不进入 HR 页面。不写应聘岗位、来源和状态，不做岗位匹配、评分或模糊自动去重。

### 17.6 第六步第五小步实际完成结果与下一步边界

已新增技能候选转换与补充信息整理模块。AI 技能只作为候选展示，HR 必须逐项勾选或全选并点击导入，才会进入可编辑的正式技能标签表单；人工标签始终保留，只去除空值和完全重复项，不做模糊或大小写合并。创建请求已显式把确认后的技能写入既有 `Candidate.tags`。

证书、自我评价和 `warnings` 已在 AI 草稿侧栏分类只读展示，使用 React 普通文本节点安全渲染；`missing_fields` 为保持界面简洁不再显示。它们都不进入 Candidate 创建请求或无关字段，完整草稿仍保留在 `Resume.parsed_snapshot`。

新增隔离测试输出 `STAGE3_RESUME_SUPPLEMENTARY_INFO_TEST_OK`；五项阶段 5 前端定向测试、TypeScript 严格检查、`git diff --check`、3922 模块生产构建和后端全量 368 项回归均通过。正式 FastAPI、Vite 与 `/api/v2/jobs` 页面依赖返回 HTTP 200；浏览器控制运行时因缺少所需路径未能连接，因此点击、截图和视觉验收仍未完成。本步没有真实调用 DeepSeek、上传简历、创建 Candidate、修改后端或数据库结构。

下一步进入第七步：准备 15～20 份去隐私样本，记录完整性、事实错误、格式失败率、响应时间、token 和费用，再完成一次受控的真实 DeepSeek + FastAPI + PostgreSQL + Vite 全链路与人工验收。未获得样本与真实调用授权前不得产生模型费用。

### 17.7 前端简历识别交互语义优化

新增候选人页面已把“AI 草稿”主流程统一改为“简历智能识别”，处理轨道使用“文件已上传 → 内容已读取 → 信息识别”，识别完成后明确提示 HR 核对再创建。原始提取文本仍完整保留，但改为默认收起的核对入口，避免和结构化识别结果形成重复功能的观感。

本步只修改前端文案、信息层级、折叠样式和界面契约测试；普通字段只补空值、冲突采用、三类经历与技能人工确认、缓存与失败恢复、Candidate 最终事务创建等规则保持不变。六项前端定向测试、TypeScript 严格检查、生产构建和 `git diff --check` 均通过；浏览器控制运行环境仍缺少所需路径，因此未完成点击、截图和最终视觉布局验收。

### 17.8 结构化识别耗时监控

后端已在 `ResumeStructureService` 内用单调时钟测量总耗时、数据准备、模型调用、结果校验和结果保存，并通过结构化成功响应的可选 `performance` 对象返回毫秒值。计时只存在于当前响应，不写入数据库或成功草稿；命中缓存时模型、校验和保存耗时为 `0`，前端会显示“缓存读取耗时”。

新增候选人页面已在识别成功区域展示本次分阶段耗时。要判断真实瓶颈，必须上传新简历，或在 HR 确认后点击“重新识别”；直接刷新或请求已有成功草稿只能测到缓存查询。后端全量 368 项、前端六项阶段 5 定向测试、TypeScript 严格检查、3922 模块生产构建和 `git diff --check` 均通过。本步没有为测量而主动产生新的 DeepSeek 调用，也没有修改数据库结构。

实测发现识别结果和耗时卡片使右侧粘性栏超过视口后，底部内容无法通过页面滚动稳定到达。现已为桌面端右栏增加视口内最大高度和独立纵向滚动，并在 `980px` 以下恢复普通页面流；界面契约测试固定这两个响应式边界，避免后续新增监控内容再次造成截断。

### 17.9 DOCX 文本框兼容补强

Resume #66 的原件经结构检查确认是有效 DOCX，`word/document.xml` 含 20 个文本框容器；原 `python-docx` 提取器只能读取普通正文和表格，因此得到空文本。已引入 `mammoth==1.12.1`：普通 DOCX 仍走原逻辑，含文本框或普通正文为空时改用 Mammoth，异常时若普通正文可用则回退；原有私有路径、文件大小、ZIP 结构、解压体积和文本长度限制保持不变。

本地隔离验证没有调用 DeepSeek、没有写数据库：Resume #66 由正式提取器得到 2,125 个字符、36 个非空段，重复段落为 0，耗时约 169 毫秒。新增文本框与失败回退测试后，DOCX 专项 11 项、后端全量 370 项全部通过。下一步是在页面对 Resume #66 执行“重新提取”；提取成功只恢复 `raw_text`，之后由 HR 主动触发的信息识别才会调用一次 DeepSeek 并产生模型耗时与费用。

### 17.10 真实页面验收与阶段收口

用户随后在真实页面上传并识别 PDF、DOCX 简历，完成原文核对、DeepSeek 信息识别、普通字段补充、三类经历与技能人工确认以及右侧结果区域滚动检查，并反馈当前测试无问题。一次 DOCX 识别总耗时约 11.96 秒，其中模型调用约 9.99 秒；项目方决定当前接受单份简历约 10～12 秒的响应速度，暂不继续进行客户端复用、提示词压缩或轻量模型替换。

原计划 15～20 份脱敏样本的字段完整性、失败率、token 和费用统计没有执行，已明确转为后续质量增强项，不记录为已完成，也不阻塞当前 MVP 进入阶段 6。OCR 同样暂缓，等真实扫描件或图片简历需求出现后再评估。阶段 5 于 2026-08-14 正式收口；后续阶段日常开发无需阅读本文，只有修改简历上传、原文提取、结构化识别、辅助填表或排查相关回归时再使用本文。
...
12. 原计划至少 15～20 份去隐私样本完成统计评估；项目方决定在真实样本操作无问题后先接受当前效果，该项转为后续质量增强，不作为当前 MVP 收口阻塞条件，也不记录为已完成。
13. 真实 DeepSeek、FastAPI、PostgreSQL、Vite 全链路和人工验收通过。

收口结论：除第 12 项按项目方决定转为非阻塞的后续质量评估外，其余当前 MVP 完成条件均已实现或验证。用户已完成真实页面测试并反馈无问题，阶段 5 于 2026-08-14 正式完成。DeepSeek 单份简历约 10～12 秒的速度当前可接受；速度优化、轻量模型替换与 OCR 暂缓。
