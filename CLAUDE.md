...
- 阶段 7 Rubric: 新岗位自动获得 standard 默认模板；HR 可选择技术/非技术模板或主动让 DeepSeek 根据岗位默认生成 5—8 个语义评分项建议，简单/复杂岗位允许 4—10 个；Python 确定性规则不计入该数量，生成结果必须经 HR 确认和版本化发布后才用于评分
- 阶段 7 规则入口: 年限、学历层级、必备/加分技能和明确关键词只能从结构化 JobRequirements 生成 Python 可执行规则；HR 手动新增的是语义项，发布后由 Prompt Builder 注入 DeepSeek 逐项评价。Python 不解析 HR 自由文本规则，DeepSeek 也不接管明确结构化比较
- 阶段 7 语义评分: DeepSeek 只评价已确认 Rubric 中的岗位专用语义项，逐项返回 `0—10/unknown`、证据、原因、置信度、优势和缺口；Python 负责确定性规则与最终加权
