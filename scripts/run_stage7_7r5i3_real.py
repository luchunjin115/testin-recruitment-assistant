...
from run_stage7_7r5i3_pricing_gate import (  # noqa: E402
...
AUTHORIZATION_PATH = (
    ...
    / "stages"
    / "stage7"
    / "2026-08-31-stage7-7r5i3-r1-real-authorization.json"
)
...
def _validate_authorization(
    ...
    for key, value in expected.items():
        if payload.get(key) != value:
            raise RuntimeError(f"CLOSE-06C 金额或单轮授权字段不一致：{key}")
    if payload.get("user_directive") != "确认 USD 2 硬上限，开始 CLOSE-06C":
        raise RuntimeError("CLOSE-06C 用户授权原文不一致")
    try:
        confirmed_at = datetime.fromisoformat(str(payload["confirmed_at"]))
    except (KeyError, TypeError, ValueError):
        raise RuntimeError("CLOSE-06C 授权时间非法") from None
    if confirmed_at.tzinfo is None:
        raise RuntimeError("CLOSE-06C 授权时间必须包含时区")
    if (
        ...
        raise RuntimeError("CLOSE-06C 授权没有绑定当前价格快照")
    if payload["monetary_cap_usd"] < pricing["requested_cap_floor_usd"]:
        raise RuntimeError("CLOSE-06C 金额上限低于价格门禁的最坏费用")
    return payload
...
def validate_zero_call_preconditions(
    *,
    ...
    pricing = validate_pricing_snapshot(
        _load_json(pricing_path, label="I3-R1 价格快照"), now=current
    )
    if pricing_tier_at(current) != pricing["selected_tier"]:
        raise RuntimeError("当前价格时段已变化，必须返回 CLOSE-06B 重新查询")
    authorization = _validate_authorization(
        _load_json(authorization_path, label="I3-R1 用户授权"),
        pricing_path=pricing_path,
        pricing=pricing,
    )
    lifecycle = validate_result_lifecycle(
        run_id=I3_RUN_ID, expected_state="i3_preflight_complete"
    )
    preflight = _load_json(I3_PREFLIGHT_PATH, label="I3-R1 preflight")
    if (
        preflight.get("stage") != I3_RUN_ID
        or preflight.get("lifecycle") != "i3_preflight_complete"
        or not preflight.get("preflight_checks", {}).get("all_passed")
    ):
        raise RuntimeError("I3-R1 preflight 身份或零调用检查不合法")
    if any(path.exists() for path in (I3_RAW_RESULT_PATH, I3_HUMAN_AUDIT_PATH, I3_FINAL_RESULT_PATH)):
        raise RuntimeError("I3-R1 正式结果路径必须在唯一 real 前为空")
    if not all(path.exists() for path in (I2_RAW_RESULT_PATH, I2_HUMAN_AUDIT_PATH, I2_FINAL_RESULT_PATH)):
        raise RuntimeError("I2 正式证据缺失，拒绝开始 I3-R1")
    return {
        "pricing": pricing,
...
def build_report_request(case: dict[str, Any]) -> dict[str, Any]:
    if case["evaluation_reference_at"] != case["application_applied_at"]:
        raise RuntimeError("I3-R1 evaluation_reference_at 必须等于 application_applied_at")
    if case["confirmed_plan_snapshot"].get("status") != "confirmed":
        raise RuntimeError("I3-R1 报告必须使用 HR 已确认计划快照")
    sanitized = screening_evaluation_service.sanitize_resume_text(case["resume_text"])
...
def _validate_runtime_settings(settings: Settings, preflight: dict[str, Any]) -> None:
    ...
    if preflight.get("execution_contract") != {
        ...
        raise RuntimeError("I3-R1 preflight 的实际执行版本已经漂移")
    if (
        ...
        raise RuntimeError("I3-R1 真实运行配置与冻结合同不一致")
...
async def _run_report_case(
    ...
    try:
        ...
        return {
            ...
            "application_applied_at": case["application_applied_at"],
...
async def zero_call_fake_preflight() -> dict[str, Any]:
    ...
    for index in I3_STABILITY_SAMPLE_INDICES:
        ...
        for run_number in range(1, I3_STABILITY_RUNS_PER_SAMPLE + 1):
            fake_payload = _fake_report_payload(
                confirmed,
                case["resume_text"],
                case["labels"]["overall_direction"],
            ...
            stability_records.append(
                await _run_report_case(
                    ...
                    index,
                    adapter,
                    case_id=f"I3-S{index:02d}-{run_number}",
                )
            )
    return {
        "plan_succeeded": sum(
    ...
    if stability:
        ...
        for index in I3_STABILITY_SAMPLE_INDICES:
            ...
            groups.append(
                {
                    "sample_index": index,
                    "case_id": I3_REPORT_PAIRS[index]["case_id"],
...
def build_raw_payload(
    *,
    ...
    stability_records = _pad_records(
        ...
        [
            {
                "case_id": f"I3-S{index:02d}-{run}",
                "sample_index": index,
                "status": "not_executed",
            }
            for index in I3_STABILITY_SAMPLE_INDICES
            for run in range(1, I3_STABILITY_RUNS_PER_SAMPLE + 1)
        ],
    )
...
def write_raw_once(path: Path, payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        raise RuntimeError("I3-R1 raw 已存在，拒绝覆盖") from None


def validate_sealed_raw() -> dict[str, Any]:
    lifecycle = validate_result_lifecycle(
        run_id=I3_RUN_ID, expected_state="i3_raw_complete"
    )
    raw = _load_json(I3_RAW_RESULT_PATH, label="I3-R1 raw")
    if (
        raw.get("stage") != I3_RUN_ID
        or raw.get("mode") != "real_raw"
        or raw.get("lifecycle") != "i3_raw_complete"
        or raw.get("preflight_sha256") != sha256_file(I3_PREFLIGHT_PATH)
        or raw.get("quality_contract_version") != I3_QUALITY_CONTRACT_VERSION
    ):
        raise RuntimeError("I3-R1 raw 身份、生命周期或冻结 preflight 不一致")
    expected_lengths = {
        "plan_records": 10,
        "report_records": 20,
    ...
    if any(
        ...
        raise RuntimeError("I3-R1 raw 的 10/20/15 固定分母不完整")
    ...
    if (
        ...
        raise RuntimeError("I3-R1 raw 的 45/90 调用审计不一致")
    if (
        ...
        or I3_HUMAN_AUDIT_PATH.exists()
        or I3_FINAL_RESULT_PATH.exists()
    ):
        raise RuntimeError("I3-R1 raw 的金额、质量停止点或零数据库合同不一致")
    return {
        "lifecycle": lifecycle,
        "raw": raw,
        "raw_sha256": sha256_file(I3_RAW_RESULT_PATH),
    }
...
async def run_real(
    *, pricing_path: Path, authorization_path: Path
    ...
    if not settings.DEEPSEEK_API_KEY.strip():
        raise RuntimeError("CLOSE-06C 已授权，但 DeepSeek API Key 未配置")
    ...
    if pricing_tier_at(datetime.now(timezone.utc)) != gate["pricing"]["selected_tier"]:
        raise RuntimeError("实例化 Adapter 前价格时段已变化，返回 CLOSE-06B")
    ...
    if payload["historical_results_before"] != payload["historical_results_after"]:
        raise RuntimeError("I3-R1 真实运行期间历史证据发生变化，拒绝写 raw")
    write_new_json(
        I3_RAW_RESULT_PATH,
        payload,
        run_id=I3_RUN_ID,
        expected_state="i3_preflight_complete",
    )
    return payload


async def main() -> None:
    parser = argparse.ArgumentParser(description="Run the single I3-R1 real raw")
    ...
    if args.monetary_cap_usd != 2.0:
        raise RuntimeError("CLOSE-06C CLI 金额必须与用户确认的 USD 2 完全一致")
    ...
    print(
        json.dumps(
            {
                "raw_path": str(I3_RAW_RESULT_PATH),
                "stage": payload["stage"],
